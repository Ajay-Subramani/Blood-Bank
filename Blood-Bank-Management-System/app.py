from flask import Flask, render_template, flash, redirect, request, url_for, session, logging # type: ignore
from flask_mysqldb import MySQL # type: ignore
from wtforms import Form, StringField, TextAreaField, PasswordField, validators, SelectField # type: ignore
...
def bloodform():
    if request.method  == 'POST':
        ...
        #Inserting values into tables
        cur.execute("INSERT INTO BLOODBANK VALUES(%s, %s, %s)",(d_id , blood_group, packets))
...
def accept():
    cur = mysql.connection.cursor()
    cur.execute("SELECT NB_GROUP, N_PACKETS FROM NOTIFICATIONS")  # Fetch only required columns
    notifications = cur.fetchall()  #  Get all rows

    if not notifications:
        flash("No notifications found!", "warning")
        return redirect(url_for('notifications'))

    for row in notifications:  #  Ensure row exists
        group = row['NB_GROUP']  #  Use dictionary keys instead of indexes
        packet = row['N_PACKETS']

        cur.execute("UPDATE BLOODBANK SET TOTAL_PACKETS = TOTAL_PACKETS - %s WHERE B_GROUP = %s", (packet, group))
        cur.execute("UPDATE NOTIFICATIONS SET RESULT = 'ACCEPTED' WHERE NB_GROUP = %s AND N_PACKETS = %s", (group, packet))
